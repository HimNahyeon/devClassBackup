...
<%
	...
	// parseRequest() 함수를 이용하여 요청메시지로부터 입력 값들을 추출한다.
	...
	while(iter.hasNext()){ //더이상 요소가 없어질때까지
		...
		if(item.isFormField()){// item이 이름 - 값 쌍으로 구성된 필드인지 확인
			...
			//사진 photo 는 getName(), get() 사진이름 , 사진 을 추출
	...
	//현재 전달 받는 입력값은 3개 id memo photo 그 중에 id memo 필드값이고 photo는 파일
	// id와 memo 는 getString(), photo는 getName() , get() 사진이름, 사진을 추출
%>
