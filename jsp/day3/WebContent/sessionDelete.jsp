...
<body>
<!-- 
클라이언트가 브라우저를 종료하면 세션은 자동으로 삭제된다.  
브라우저 종료를 하지 않더라도 주기적으로 세션의 상태를 확인하여
특정 시간(기본: 30분)동안 클라이언트로부터 어떤 요청도 발생하지 않으면
세션을 삭제한다. 
-->
...
out.print("클라이언트의 해당 세션 마지막 접근 시간 = " + session.getLastAccessedTime()
			+ "초<br>");
out.print("클라이언트의 세션 유지 시간 = " + (session.getLastAccessedTime()
			-session.getCreationTime()) + "초<br>");
